...
// 绘制文字
  ...
  onGetCaptcha = async () => {
	...
	const { mobile } = this.state;//获取手机号码
	const value = form.getFieldValue('sendcode');//获取用户输入的验证码
    let code = this.state.code;//获取系统生成的验证码
    /*await form.validateFields({ force: true }, (err, values) => {
      if(!(code.toLowerCase() === (values.sendcode).toLowerCase())) {
